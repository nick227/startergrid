-- Restore generic category inventory tables and marketplace listing links.
-- The application still uses these models for non-automotive inventory.
...
ALTER TABLE `CategoryInventoryItem`
  ADD CONSTRAINT `CategoryInventoryItem_dealershipId_fkey`
  FOREIGN KEY (`dealershipId`)
  REFERENCES `DealershipProfile`(`id`)
  ON DELETE CASCADE
  ON UPDATE CASCADE;

ALTER TABLE `CategoryInventoryItemMedia`
  ADD CONSTRAINT `CategoryInventoryItemMedia_itemId_fkey`
  FOREIGN KEY (`itemId`)

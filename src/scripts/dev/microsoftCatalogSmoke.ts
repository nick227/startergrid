/**
 * Microsoft Automotive Ads (Merchant Center) catalog smoke test.
 *
 * Prerequisites:
 *   - Microsoft OAuth connected (run smoke:microsoft-oauth first)
 *   - MICROSOFT_CLIENT_ID / MICROSOFT_CLIENT_SECRET set
 *   - --store-id=<id>  (Merchant Center store ID from Microsoft Merchant Center)
 *   - At least one vehicle in the DB
 *
 * Usage:
 *   npm run smoke:microsoft-catalog -- --store-id=<id>
 *   npm run smoke:microsoft-catalog -- --store-id=<id> --dealer-id=<id>
 *   npm run smoke:microsoft-catalog -- --store-id=<id> --skip-delete
 */
...
const PLATFORM_SLUG = 'microsoft-automotive-ads';
...
async function run() {
  ...
  console.log('\n══════════════════════════════════════════════════════════');
  console.log('  Microsoft Automotive Ads (Merchant Center) Catalog Smoke Test');
  console.log('══════════════════════════════════════════════════════════');
  ...
  if (!storeId) {
    fail('--store-id required', 'Find it in Microsoft Merchant Center → Account settings');
    await prisma.$disconnect(); process.exit(1);
  }
  pass('--store-id', storeId);
  ...
  const msClient = new MicrosoftOAuthClient();
  let token: string;
  try {
    token = await CredentialStore.withFreshToken(prisma, dealerId, PROVIDER, msClient);
    ...
    info('Run npm run smoke:microsoft-oauth first to connect Microsoft OAuth.');
    await prisma.$disconnect(); process.exit(1);
  }

  // ── Phase 4: Verify Merchant Center access ────────────────────────────────

  section('PHASE 4 · Verify Merchant Center store access');

  try {
    const store = await MicrosoftMerchantClient.getStore(token, storeId);
    pass('Merchant Center accessible', `"${store.name}" (merchantId=${store.merchantId})`);
  } catch (err: unknown) {
    fail('Merchant Center accessible', err instanceof Error ? err.message : String(err));
    info('Ensure the Microsoft account has Merchant Center access and the store ID is correct.');
    await prisma.$disconnect(); process.exit(1);
  }
  ...
  try {
    syncResult = await bridge.upsertItems(token, storeId, items);
  ...
  try {
    const config = await CatalogSyncStore.upsertConfig(prisma, dealerId, PLATFORM_SLUG, storeId);
    await CatalogSyncStore.markSynced(prisma, dealerId, PLATFORM_SLUG, vehicles.length);
    pass('Catalog config stored', `id=${config.id}, storeId=${storeId}`);
  } catch (err: unknown) {
  ...
  if (skipDelete) {
    ...
    info(`Products from this test remain in Microsoft Merchant Center ${storeId}.`);
  } else {
    ...
    try {
      await bridge.deleteItem(token, storeId, deleteTarget);
  ...
  console.log(`\n${'═'.repeat(60)}`);
  if (failures === 0) {
    console.log('  ✅  All checks passed — Microsoft Automotive Ads catalog sync is healthy');

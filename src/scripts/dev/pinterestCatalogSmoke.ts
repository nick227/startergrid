/**
 * Pinterest Shopping Ads catalog smoke test.
 *
 * Prerequisites:
 *   - Pinterest OAuth connected (run smoke:pinterest-oauth first)
 *   - PINTEREST_CLIENT_ID / PINTEREST_CLIENT_SECRET set
 *   - --catalog-id=<id>  (Pinterest catalog ID from Pinterest Business Hub → Catalogs)
 *   - At least one vehicle in the DB
 *
 * Usage:
 *   npm run smoke:pinterest-catalog -- --catalog-id=<id>
 *   npm run smoke:pinterest-catalog -- --catalog-id=<id> --dealer-id=<id>
 *   npm run smoke:pinterest-catalog -- --catalog-id=<id> --skip-delete
 */
...
const PLATFORM_SLUG = 'pinterest-shopping-ads';
...
async function run() {
  ...
  console.log('\n══════════════════════════════════════════════════════════');
  console.log('  Pinterest Shopping Ads Catalog Smoke Test');
  console.log('══════════════════════════════════════════════════════════');
  ...
  if (!catalogId) {
    fail('--catalog-id required', 'Find it in Pinterest Business Hub → Catalogs');
  ...
  const pinClient = new PinterestOAuthClient();
  let token: string;
  try {
    token = await CredentialStore.withFreshToken(prisma, dealerId, PROVIDER, pinClient);
    ...
    info('Run npm run smoke:pinterest-oauth first to connect Pinterest OAuth.');
    await prisma.$disconnect(); process.exit(1);
  }

  // ── Phase 4: Upsert vehicles ──────────────────────────────────────────────

  section('PHASE 4 · Upsert vehicles to Pinterest catalog');
  ...
  info('Note: Pinterest catalog batches are async — you will receive a batch_id.');

  let syncResult: { accepted: number; rejected: number; handles?: string[] };
  try {
  ...
  if (skipDelete) {
    ...
    info(`Products from this test remain in Pinterest catalog ${catalogId}.`);
  } else {
    section('PHASE 6 · Delete one product from Pinterest catalog');
    ...
    try {
      ...
      pass('Product delete request sent', `itemId=${deleteTarget}`);
    } catch (err: unknown) {
  ...
  console.log(`\n${'═'.repeat(60)}`);
  if (failures === 0) {
    console.log('  ✅  All checks passed — Pinterest Shopping Ads catalog sync is healthy');
  } else {

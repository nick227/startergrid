/**
 * X Dynamic Product Ads catalog smoke test.
 *
 * Prerequisites:
 *   - X OAuth connected (run smoke:x-oauth first — not yet wired, connect manually)
 *   - X_CLIENT_ID / X_CLIENT_SECRET set
 *   - Approved X Ads API developer account with DPA catalog access
 *   - --catalog-id=<id>  (X DPA catalog ID from X Ads Manager → Assets → Catalogs)
 *   - At least one vehicle in the DB
 *
 * Usage:
 *   npm run smoke:x-catalog -- --catalog-id=<id>
 *   npm run smoke:x-catalog -- --catalog-id=<id> --dealer-id=<id>
 *   npm run smoke:x-catalog -- --catalog-id=<id> --skip-delete
 *
 * NOTE: Live validation is blocked until X_CLIENT_ID, X_CLIENT_SECRET, and an
 *       approved X Ads API developer account with DPA catalog access are available.
 */
...
const PLATFORM_SLUG = 'x-dynamic-product-ads';
...
async function run() {
  ...
  console.log('\n══════════════════════════════════════════════════════════');
  console.log('  X Dynamic Product Ads Catalog Smoke Test');
  console.log('══════════════════════════════════════════════════════════');
  ...
  if (!catalogId) {
    fail('--catalog-id required', 'Find it in X Ads Manager → Assets → Catalogs');
  ...
  let token: string;
  try {
    token = await CredentialStore.withFreshToken(prisma, dealerId, PROVIDER, xClient);
    ...
    info('Connect X OAuth first (no dedicated oauth smoke script yet — use the connect flow).');
    await prisma.$disconnect(); process.exit(1);
  }

  // ── Phase 4: Upsert vehicles ──────────────────────────────────────────────

  section('PHASE 4 · Upsert vehicles to X DPA catalog');
  ...
  if (skipDelete) {
    ...
    info(`Products from this test remain in X DPA catalog ${catalogId}.`);
  } else {
    section('PHASE 6 · Delete one product from X DPA catalog');
  ...
  console.log(`\n${'═'.repeat(60)}`);
  if (failures === 0) {
    console.log('  ✅  All checks passed — X Dynamic Product Ads catalog sync is healthy');
  } else {

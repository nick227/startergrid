    ...
    try {
      ...
      for (const app of apps) {
        ...
        blockedDealers.push({
          ...
          nextAction: app.nextAction || 'Resolve platform requirements',
          status: app.status,
          firstSeenAt: app.createdAt.toISOString(),
          lastSeenAt: app.updatedAt.toISOString(),
          affectedCount: null,
      ...
      for (const [key, items] of failedGroups) {
        ...
        blockedDealers.push({
          ...
          nextAction: 'Resolve platform error or retry sync',
          status: 'FAILED',
          firstSeenAt: oldest.createdAt.toISOString(),
          lastSeenAt: newest.updatedAt.toISOString(),
        ...
        if (isAddressable) {
          ...
          if (!isGeocoded) {
            blockedDealers.push({
              ...
              platformName: 'All Platforms',
              source: 'geo',
              reason: 'Dealership profile is missing geocoding coordinates',
              nextAction: 'Verify and geocode dealership rooftop address',
              status: 'NOT_CONFIGURED',
              firstSeenAt: dealer.createdAt.toISOString(),
              lastSeenAt: dealer.updatedAt.toISOString(),
              affectedCount: null,

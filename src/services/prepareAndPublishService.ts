...
import { dbDealershipToPayload } from './readinessRunService.js';
...
export function derivePublishState(opts: {
  ...
  const { platform, readiness, applicationStatus, latestQueueItemStatus, hasSubmissionAttempt } = opts;
...
export async function runPrepareAndPublish(
  ...
  const platforms: PlatformPublishResult[] = targets.map(platform => {
    ...
    const detail = stateDetail(state, platform, report, queueItem?.scheduledFor ?? null);
...
function stateDetail(
  ...
  switch (state) {
    ...
    case 'Partner Required':return `Commercial agreement with ${platform.name} required before submission.`;

...
import { VideoItemFields, videoFieldsToData, type VideoFields } from './VideoItemFields.tsx';
...
function emptyForm(): VideoFields {
  return {
    title: '',
    creator: '',
    genre: '',
...
export function VideoEntryPanel({ dealerId, onCreated }: Props) {
  const [phase, setPhase] = useState<Phase>({ step: 'idle' });
  const [form, setForm] = useState<VideoFields>(emptyForm());
  const [price, setPrice] = useState('');
  const [createError, setCreateError] = useState<string | null>(null);

  const isCreating = phase.step === 'creating';
  const priceCents = priceToCents(price);
  const isFormValid = form.title.trim().length > 0 && form.creator.trim().length > 0 && priceCents >= 0;

  const handleCreate = async () => {
    setPhase({ step: 'creating' });
    setCreateError(null);
    try {
      const data = videoFieldsToData(form);
    ...
    return (
      <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between">
        <p className="text-xs text-green-700 font-medium">
          Video added — Stock #{phase.stockNumber}
        </p>
  ...
  return (
    <div className="space-y-3">
      <VideoItemFields fields={form} onChange={setForm} readOnly={isCreating} />

      <div>
        <label className="block text-[11px] font-semibold text-ink-muted mb-1">
          Price <span className="text-red-500">*</span>{' '}
          <span className="text-ink-faint font-normal">(enter 0 for free)</span>
        </label>

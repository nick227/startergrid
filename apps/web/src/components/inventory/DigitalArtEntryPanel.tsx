...
import { DigitalArtItemFields, digitalArtFieldsToData, type DigitalArtFields } from './DigitalArtItemFields.tsx';
...
function emptyForm(): DigitalArtFields {
  return {
    title: '',
    artist: '',
    series: '',
...
export function DigitalArtEntryPanel({ dealerId, onCreated }: Props) {
  const [phase, setPhase] = useState<Phase>({ step: 'idle' });
  const [form, setForm] = useState<DigitalArtFields>(emptyForm());
  ...
  const handleCreate = async () => {
    ...
    setCreateError(null);
    try {
      const data = digitalArtFieldsToData(form);
    ...
    return (
      <div className="p-3 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between">
        <p className="text-xs text-green-700 font-medium">
          Artwork added — Stock #{phase.stockNumber}
        </p>
  ...
  return (
    <div className="space-y-3">
      <DigitalArtItemFields fields={form} onChange={setForm} readOnly={isCreating} />

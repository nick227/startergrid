      ...
      <header className="sticky top-0 z-20 border-b border-silver-200 bg-surface-card/95 backdrop-blur">
        <nav className="mx-auto flex h-14 max-w-6xl items-center justify-between gap-2 px-4 sm:px-6" aria-label="Marketplace">
          <div className="flex min-w-0 shrink items-center gap-2 sm:gap-3">
            <a href={homeHref} className="mp-focus shrink-0 text-base font-bold tracking-tight text-navy-900 hover:text-navy-800">
              {siteTitle}
            </a>
            {showCategoryNav && (
              <a href={sitesHref()} className="mp-focus hidden text-sm font-medium text-ink-muted hover:text-ink-heading sm:block">
                All sites
              </a>
            )}
            {backHref && (
